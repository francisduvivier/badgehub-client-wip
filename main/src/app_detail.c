...
static void detail_key_event_handler(lv_event_t * e); // New handler for detail page navigation

// A simple struct to pass both buttons to the event handler
typedef struct {
    lv_obj_t *btn_back;
...
void create_app_detail_view(const char* slug, int revision) {
    ...
    if (details) {
        ...
        // Free the nav_data when the container is deleted
...
static void detail_view_delete_event_handler(lv_event_t * e) {
    // This handler now frees either the project_detail_t or the detail_nav_t
...
/**
 * @brief Handles key events for the detail page buttons to allow navigation.
 */
static void detail_key_event_handler(lv_event_t * e) {
    ...
    if (key == LV_KEY_UP) {
        if (focused_btn == nav_data->btn_back) {
            // At the top, wrap to the bottom
            lv_group_focus_obj(nav_data->btn_install);
        } else {
            // Go up to the back button
            lv_group_focus_obj(nav_data->btn_back);
        }
    } else if (key == LV_KEY_DOWN) {
        if (focused_btn == nav_data->btn_install) {
            // At the bottom, wrap to the top
            lv_group_focus_obj(nav_data->btn_back);
        } else {
            // Go down to the install button
            lv_group_focus_obj(nav_data->btn_install);
        }
    }
}
